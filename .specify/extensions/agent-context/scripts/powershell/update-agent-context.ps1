...
if ($null -eq $Options) {
    ...
    foreach ($candidate in @('python', 'python3')) {
        if (Get-Command $candidate -ErrorAction SilentlyContinue) {
            # Verify it is Python 3. Probe inside try/catch so the Windows Store python
            # stub (which writes to stderr) doesn't throw a terminating NativeCommandError
            # under $ErrorActionPreference='Stop' and abort the whole script.
            try {
    ...
    if ($pythonCmd) {
        # Write the helper to a temp .py file rather than passing a multi-line `-c`
        # argument: Windows PowerShell 5.1 mangles multi-line native-command arguments,
        # truncating the script mid-statement (SyntaxError). Running a file is reliable.
        $pyHelper = @'
import json
import sys
try:
...
'@
        $tmpPy = Join-Path ([System.IO.Path]::GetTempPath()) "speckit-agent-context-$PID.py"
        try {
            [System.IO.File]::WriteAllText($tmpPy, $pyHelper, (New-Object System.Text.UTF8Encoding($false)))
